...
try:
    ...
    logger.warning("Blockchain integration not available for challenges")
        ...
        try:
            ...
            if blockchain_user:
                ...
                today_challenges = session.query(BlockchainUserChallenge).filter(
                    BlockchainUserChallenge.user_id == blockchain_user.id,
...
async def start_challenge(
    ...
    try:
        ...
        if not BLOCKCHAIN_INTEGRATION:
            # Use Supabase instead of mock data
            try:
                # Record challenge start in Supabase
                challenge_data = {
                    "user_id": user_id,
                ...
                response = supabase_service.client.table("user_challenges").insert(challenge_data).execute()
                ...
                logger.error(f"Failed to start challenge in Supabase: {e}")
                raise HTTPException(status_code=500, detail="Failed to start challenge")
        ...
        try:
            ...
            challenge = session.query(BlockchainChallenge).filter(
                BlockchainChallenge.id == int(request.challenge_id),
                BlockchainChallenge.is_active == True
            ).first()
...
async def complete_challenge(
    ...
    try:
        ...
        if not BLOCKCHAIN_INTEGRATION:
            # Use Supabase instead of mock data
            try:
                # Update challenge status to completed in Supabase
                current_time = int(time.time())
                update_data = {
                    "status": "completed",
                ...
                # Update the user's challenge record
                response = supabase_service.client.table("user_challenges").update(update_data).eq("user_id", user_id).eq("challenge_id", request.challenge_id).execute()

                if response.data:
                    # Also update user's total points
                    user_response = supabase_service.client.table("profiles").select("points").eq("user_id", user_id).execute()
                    current_points = user_response.data[0]["points"] if user_response.data else 0
                    new_points = current_points + 100

                    supabase_service.client.table("profiles").update({"points": new_points}).eq("user_id", user_id).execute()
                ...
                logger.error(f"Failed to complete challenge in Supabase: {e}")
        ...
        try:
            ...
            user_challenge = session.query(BlockchainUserChallenge).filter(
                BlockchainUserChallenge.user_id == blockchain_user.id,
                BlockchainUserChallenge.challenge_id == int(request.challenge_id),
                BlockchainUserChallenge.date == today
            ).first()
