...
export default function ProfileSettings() {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      // Save basic profile info to backend
      ...
      if (!profileResponse.success) {
        console.warn("[ProfileSettings] Failed to save profile:", profileResponse.error);
      }

      // Save medical info to backend
      const medicalResponse = await apiService.updateMedicalInfo({
        blood_type: bloodType,
        allergies,
        medications,
      ...
      if (!medicalResponse.success) {
        console.warn("[ProfileSettings] Failed to save medical info:", medicalResponse.error);
      }

      // Save mood to backend
      const moodResponse = await apiService.updateMood(mood);
      if (!moodResponse.success) {
        console.warn("[ProfileSettings] Failed to save mood:", moodResponse.error);
      }

      // Persist profile fields to local storage as backup
      await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(merged));

      // ✅ Earn: Set the mood today (+5) — once per day
      await awardEarnOncePerDay("set_mood_today", 5);

      console.log("[ProfileSettings] Profile saved successfully to backend and local storage");
    } catch (error) {
