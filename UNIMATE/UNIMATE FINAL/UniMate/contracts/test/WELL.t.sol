...
contract WELLTest is Test {
    ...
    function testInitialState() public {
        ...
        // Verify owner has MINTER_ROLE
        bytes32 MINTER_ROLE = keccak256("MINTER_ROLE");
        assertTrue(well.hasRole(MINTER_ROLE, owner));
        // Verify relayer has MINTER_ROLE
        assertTrue(well.hasRole(MINTER_ROLE, relayerWallet));
    }
    ...
    function testMintFailsForNonMinter() public {
        // Bob doesn't have MINTER_ROLE, so this should revert
        vm.prank(bob);
        vm.expectRevert();
    ...
    function testPauseFailsForNonPauser() public {
        // Bob doesn't have PAUSER_ROLE, so this should revert
        vm.prank(bob);
        vm.expectRevert();
        well.pause();
    }
