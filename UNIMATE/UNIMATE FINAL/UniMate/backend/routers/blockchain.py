...
from config import (
    ...
    ACH, RS, BICONOMY_BUNDLER_URL, ALLOWED_ORIGINS, settings
)
...
from models import db, SmartAccountInfo
...
def get_user_id_for_rate_limit(request: Request):
    ...
    Security: Properly verifies JWT signature to prevent user ID spoofing.
    Falls back to IP address if JWT is invalid or missing.
    """
    try:
        auth_header = request.headers.get("Authorization", "")
        if auth_header.startswith("Bearer "):
            token = auth_header.split(" ", 1)[1]
            # For Supabase JWT - verify signature properly
            try:
                ...
                payload = jwt.decode(
                    ...
                    algorithms=["HS256"],
                    options={"verify_signature": True}  # ✅ SECURE: Always verify
                )
                return payload.get("sub", get_remote_address(request))
            except jwt.ExpiredSignatureError:
                logger.warning("Expired JWT token for rate limiting")
                return get_remote_address(request)
            except jwt.InvalidTokenError as e:
                logger.warning(f"Invalid JWT token for rate limiting: {e}")
                return get_remote_address(request)
...
def balance(address: str):
    """Get WELL token balance for a specific address (public endpoint)"""
    try:
...
@router.get("/balance")
def get_user_balance(current_user: dict = Depends(get_authenticated_user)):
    """Get authenticated user's WELL token balance (frontend-compatible endpoint)"""
    session = db()
    try:
        # Query user's smart account address from database
        ...
        if not account_info:
            raise HTTPException(
                status_code=404,
                detail="Smart account not found. Please create a smart account first."
            )

        # Get balance for user's smart account address
        try:
            addr = Web3.to_checksum_address(account_info.smart_account_address)
            wei = token.functions.balanceOf(addr).call()
            human = wei / (10 ** decimals)
            return {
                "address": addr,
            ...
            logger.error(f"Failed to get balance for address {account_info.smart_account_address}: {e}")
            raise HTTPException(status_code=500, detail="Failed to fetch token balance")

    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Failed to get user balance for user {current_user['sub']}: {e}")
...
async def mint_gasless(body: MintGaslessBody, request: Request = None):
    """
    ...
    # Execute via Node.js gasless mint script in aa-test directory
    aa_test_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "aa-test")
    amount_str = str(float(body.amount))  # Float WELL units (e.g., 0.5, 1.0, 5.0)

    try:
        logger.info(f"Executing Biconomy gasless mint script in: {aa_test_dir}")
        ...
        try:
            ...
            if proc.returncode != 0:
                logger.error(f"Biconomy gasless mint script failed: {stderr_text}")
                error_msg = stderr_text[-400:] if stderr_text else "Unknown Biconomy error"
                raise HTTPException(500, detail=f"Gasless transaction failed: {error_msg}")

        except asyncio.TimeoutError:
            proc.kill()
            await proc.wait()
            raise HTTPException(408, "Gasless transaction timeout after 2 minutes")
        ...
        logger.error(f"Biconomy mint script timeout for {client_ip}")
        raise HTTPException(408, detail="Gasless transaction timeout")
    except Exception as e:
        logger.error(f"Biconomy mint script execution error: {e}")
...
async def get_smart_account_address(request: SmartAccountRequest, http_request: Request):
    ...
    try:
        ...
        # Create temporary signer data using secure temp file (prevents race conditions)
        signer_data = {
            "signerAddress": request.signer_address,
            "chainId": request.chain_id
        }

        # Use NamedTemporaryFile for secure, unique temp file
        signer_file_obj = tempfile.NamedTemporaryFile(
            mode='w',
            suffix='.json',
            dir=aa_test_dir,
            delete=False
        )
        signer_file = signer_file_obj.name
        json.dump(signer_data, signer_file_obj)
        signer_file_obj.close()

        # Execute Node.js script to get smart account address (async)
        node_script = f"""
        ...
        # Guaranteed cleanup: always remove temp file
        if signer_file and os.path.exists(signer_file):
            try:
                os.remove(signer_file)
            except Exception as cleanup_error:
                logger.warning(f"Failed to cleanup temp file {signer_file}: {cleanup_error}")
...
async def execute_batch_transaction(request: BatchExecuteRequest, http_request: Request):
    ...
    try:
        ...
        # Verify aa-test directory exists
        if not os.path.exists(aa_test_dir):
            logger.error(f"aa-test directory not found: {aa_test_dir}")
            raise HTTPException(503, "ERC-4337 executor not configured. Please run: cd backend/aa-test && npm install")
        ...
        session = db()
        try:
            smart_account_info = session.query(SmartAccountInfo).filter(
                SmartAccountInfo.smart_account_address == request.smart_account_address
            ).first()

            if not smart_account_info or not smart_account_info.encrypted_private_key:
                raise HTTPException(503, "Smart account private key not found")

            # Decrypt the user's private key
            user_private_key = decrypt_private_key(smart_account_info.encrypted_private_key)
            logger.info(f"✅ Retrieved and decrypted private key for smart account {request.smart_account_address[:10]}...")

        finally:
            session.close()

        # ============================================
        # STEP 1: Check and deploy smart account if needed
        # ============================================
        logger.info(f"🔍 Checking if smart account {request.smart_account_address} is deployed...")
        ...
        deploy_file_obj = tempfile.NamedTemporaryFile(
            mode='w',
            suffix='-deploy.json',
            dir=aa_test_dir,
            delete=False
        )
        deploy_file = deploy_file_obj.name
        json.dump(deploy_config, deploy_file_obj)
        deploy_file_obj.close()

        deploy_result_file = deploy_file.replace('.json', '-result.json')

        try:
            # Execute deploy-smart-account.js
            logger.info(f"🚀 Running deploy check: deploy-smart-account.js {deploy_file}")

            deploy_proc = await asyncio.create_subprocess_exec(
                "node", "deploy-smart-account.js", deploy_file,
                cwd=aa_test_dir,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )

            deploy_stdout, deploy_stderr = await asyncio.wait_for(
                deploy_proc.communicate(),
                timeout=120
            )

            deploy_stdout_text = deploy_stdout.decode()
            deploy_stderr_text = deploy_stderr.decode()

            logger.info(f"📤 Deploy stdout:\n{deploy_stdout_text}")
            if deploy_stderr_text:
                logger.warning(f"⚠️ Deploy stderr:\n{deploy_stderr_text}")

            # Read deployment result
            if os.path.exists(deploy_result_file):
                ...
                if not deploy_result.get("success", False):
                    ...
                    logger.error(f"❌ Smart account deployment failed: {error_msg}")
                    raise HTTPException(503, f"Smart account deployment failed: {error_msg}")

                # Get the actual derived address from Biconomy SDK
                actual_smart_account = deploy_result.get("smart_account_address")

                # Update database if address differs (factory mismatch)
                if actual_smart_account and actual_smart_account.lower() != request.smart_account_address.lower():
                    logger.warning(f"⚠️  Smart account address mismatch detected!")
                    logger.warning(f"   Database: {request.smart_account_address}")
                    logger.warning(f"   Derived:  {actual_smart_account}")
                    logger.warning(f"   Updating database with correct address...")

                    # Update database with correct address
                    session_update = db()
                    try:
                        smart_account_update = session_update.query(SmartAccountInfo).filter(
                            SmartAccountInfo.smart_account_address == request.smart_account_address
                        ).first()

                        if smart_account_update:
                            smart_account_update.smart_account_address = actual_smart_account
                            smart_account_update.updated_at = datetime.utcnow()
                            session_update.commit()
                            logger.info(f"✅ Database updated with correct smart account address: {actual_smart_account}")

                            # Update request object for batch execution
                            request.smart_account_address = actual_smart_account
                        else:
                            logger.error("❌ Could not find smart account record to update")
                    except Exception as update_err:
                        session_update.rollback()
                        logger.error(f"❌ Failed to update smart account address: {update_err}")
                    finally:
                        session_update.close()

                if deploy_result.get("already_deployed"):
                    logger.info(f"✅ Smart account already deployed at {actual_smart_account}")
                else:
                    tx_hash = deploy_result.get("transaction_hash", "N/A")
                    logger.info(f"✅ Smart account deployed successfully! Tx: {tx_hash}")
            else:
                logger.error("❌ Deployment result file not found")
                raise HTTPException(503, "Deployment check failed - no result file")

        finally:
            # Clean up deployment files
            try:
                if deploy_file and os.path.exists(deploy_file):
                    os.unlink(deploy_file)
                if deploy_result_file and os.path.exists(deploy_result_file):
                    os.unlink(deploy_result_file)
            except Exception as cleanup_err:
                logger.warning(f"Failed to cleanup deployment files: {cleanup_err}")

        logger.info("✅ Smart account deployment check complete. Proceeding with batch transaction...")

        # ============================================
        # STEP 2: Execute batch transaction
        # ============================================
        # Prepare the batch transaction data with snake_case for Node.js
        batch_data = {
            "smart_account_address": request.smart_account_address,
            "calls": [call.dict() for call in request.calls],
            "chain_id": request.chain_id,
            "user_private_key": user_private_key  # Pass the decrypted key to Node.js
        }

        # Write batch data to secure temporary file (prevents race conditions)
        batch_file_obj = tempfile.NamedTemporaryFile(
            mode='w',
            suffix='.json',
            dir=aa_test_dir,
            delete=False
        )
        batch_file = batch_file_obj.name
        json.dump(batch_data, batch_file_obj)
        batch_file_obj.close()

        logger.info(f"📝 Batch config written to: {batch_file}")

        # Result file path (Node.js will create this)
        result_file = batch_file.replace('.json', '-result.json')

        # Execute via dedicated Node.js script (async)
        logger.info(f"🚀 Executing Node.js script: execute-batch.js {batch_file}")

        proc = await asyncio.create_subprocess_exec(
            "node", "execute-batch.js", batch_file,
            cwd=aa_test_dir,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        ...
        try:
            # Wait up to 2 minutes for batch execution
            stdout, stderr = await asyncio.wait_for(
                proc.communicate(),
                timeout=120
            )
            ...
            if stdout_text:
                logger.info(f"📤 Node.js stdout:\n{stdout_text}")
            if stderr_text:
                logger.warning(f"⚠️ Node.js stderr:\n{stderr_text}")

            # Read result from result file (more reliable than parsing stdout)
            if os.path.exists(result_file):
                ...
                if success:
                    ...
                    logger.info(f"✅ Batch execution successful!")
                    logger.info(f"   UserOp Hash: {user_op_hash}")
                    logger.info(f"   Transaction Hash: {transaction_hash}")
                    logger.info(f"   Block: {block_number}, Gas: {gas_used}")
                    ...
                    logger.error(f"❌ Batch execution failed: {error}")
                    return BatchExecuteResponse(
                        user_op_hash="",
                        transaction_hash=None,
                        success=False,
                        error=error or "Unknown error from Node.js executor"
                    )
            else:
                # Result file not created - Node.js script failed early
                error_msg = stderr_text or "Node.js script failed to create result file"
            ...
            proc.kill()
            await proc.wait()
            logger.error("Batch execution timeout after 2 minutes")
            return BatchExecuteResponse(
                user_op_hash="",
                transaction_hash=None,
                success=False,
                error="Batch execution timeout after 2 minutes"
            )
        ...
        # Guaranteed cleanup: always remove temp files
        if batch_file and os.path.exists(batch_file):
            try:
                os.remove(batch_file)
                logger.debug(f"Cleaned up batch config file: {batch_file}")
            except Exception as cleanup_error:
                logger.warning(f"Failed to cleanup batch file {batch_file}: {cleanup_error}")

        if result_file and os.path.exists(result_file):
            try:
                os.remove(result_file)
                logger.debug(f"Cleaned up result file: {result_file}")
            except Exception as cleanup_error:
                logger.warning(f"Failed to cleanup result file {result_file}: {cleanup_error}")
...
async def aa_wellness_redeem(body: WellnessRedeemBody, http_request: Request):
    ...
    try:
        ...
        # Create signature for batch transaction
        # Format: smart_account_address|calls_data|chain_id|timestamp
        import hmac
        import hashlib
        calls_str = "|".join([f"{call.to}:{call.data}:{call.value}" for call in calls])
        batch_raw = f"{body.smart_account_address}|{calls_str}|80002|{body.ts}"
        batch_sig = hmac.new(
            API_SECRET.encode(),
            batch_raw.encode(),
            hashlib.sha256
        ...
        batch_request = BatchExecuteRequest(
            ...
            chain_id=80002,
            ts=body.ts,
            sig=batch_sig  # Use the new batch-specific signature
        )

        result = await execute_batch_transaction(batch_request, http_request)
...
def redeem(body: RedeemBody, request: Request):
    ...
    # Demo mode: only owner can redeem (DISABLED for production use)
    # Allow all users with smart accounts to redeem
    owner = w3.eth.account.from_key(OWNER_PK if OWNER_PK.startswith("0x") else "0x"+OWNER_PK)
...
# --- Persistence: Import models from centralized models.py ---
from models import (
    db, init_database,
    UserOperation, Voucher,
    Challenge, UserChallenge, UserPoints
)

# Initialize database on startup
init_database()

# --- Legacy /chain/auth endpoints removed ---
# Use /users/profile (routers/profile.py) and /auth/accounts (routers/core.py) instead
    ...
    try:
        ...
        if batch_file and os.path.exists(batch_file):
            try:
                ...
                logger.warning(f"Failed to cleanup temp file {batch_file}: {cleanup_error}")
...
async def aa_get_status(
    ...
    try:
        ...
        try:
            ...
            # Query status via Node.js script (async)
            proc = await asyncio.create_subprocess_exec(
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    logger.error(f"Status query failed: {stderr_text}")
                ...
                proc.kill()
                await proc.wait()
                logger.warning(f"UserOp status query timeout for {user_op_hash}")
                # Return database status on timeout
                return AAStatusResponse(
                    status=user_op.status,
                    entry_point_tx_hash=user_op.entry_point_tx_hash,
                    revert_reason=user_op.revert_reason
